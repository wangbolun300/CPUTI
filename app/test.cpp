...
void all_ccd_run(const std::vector<std::array<std::array<Scalar, 3>, 8>> &V, bool is_edge,
                 std::vector<int> &result_list, double &run_time, std::vector<Scalar> &time_impact, int parallel_nbr)
{
...
{
    std::string folder = std::string(getenv("HOME")) + "/test/"; // this is the output folder
